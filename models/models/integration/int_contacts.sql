WITH hubspot_contacts AS (
    ...
    SELECT MAX(hubspot_contact_id) AS hubspot_contact_id, MAX(rds_contact_id) AS rds_contact_id, first_name, last_name, MAX(customer_phone) AS customer_phone, 
    MAX(hubspot_company_id) AS hubspot_company_id, MAX(rds_company_id) AS rds_customers
    FROM union_contacts
